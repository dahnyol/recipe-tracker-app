...
module.exports = {
  ...
  getFavorites: async (req, res) => {
    try {
      //Find all the favorite's with user id that match the logged in user id
      const recipes = await Favorite.find({ user: req.user.id })
      //Grab all recipe values
      .populate('recipe');
  ...
  getRecipe: async (req, res) => {
    try {
      ...
      //example url: http://localhost:5000/post/2f5e8acf3as5d
      //id === 2f5e8acf3as5d
  ...
  createRecipe: async (req, res) => {
    try {
      ...
      //media is stored on cloudinary - the above request responds with a url to media and the media id to target for delete
      await Recipe.create({
        name: req.body.name,
